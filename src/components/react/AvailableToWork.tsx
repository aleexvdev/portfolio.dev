interface AvailableToWorkProps {
  text: string;
}

export const AvailableToWork = ({ text }: AvailableToWorkProps) => {
  return (
    <div className="relative inline-flex h-full w-max overflow-hidden rounded-lg p-0.5">
      ...
      <div className="flex h-full w-full items-center justify-center space-x-3 rounded-lg px-3 py-2 backdrop-blur-3xl bg-[#0C0A09]">
        ...
        <span className="inline-flex text-sm font-medium text-nowrap md:text-base lg:text-lg text-white">
          {text}
        </span>
      </div>
    </div>
