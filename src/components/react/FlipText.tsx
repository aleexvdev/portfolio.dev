...
import { type ElementType, isValidElement,Children } from "react";
...
export function FlipText({
  ...
  const extractText = (node: React.ReactNode): string => {
    ...
    if (isValidElement(node)) {
      // @ts-ignore - Accede a props.value si existe
      return node.props?.value || Children.toArray(node.props.children).map(extractText).join("");
    }
    return "";
  };
